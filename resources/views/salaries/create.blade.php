...
<div class="container mt-4">
    <div class="card">
        <div class="card-header bg-primary text-white">
            <h4 class="mb-0"><i class="fas fa-money-bill-wave"></i> Tambah Data Gaji</h4>
        </div>
        <div class="card-body">
            <form action="{{ route('salaries.store') }}" method="POST">
                    ...
                    <select class="form-control @error('karyawan_id') is-invalid @enderror" 
                        ...
                        @foreach($employees as $emp)
                            <option value="{{ $emp->id }}" {{ old('karyawan_id') == $emp->id ? 'selected' : '' }}>
                                ...
                                Gaji Pokok: Rp {{ number_format($emp->position->gaji_pokok ?? 0, 0, ',', '.') }})
                            </option>
                        @endforeach
                    </select>
                    @error('karyawan_id')
                ...
                <div class="mb-3">
                    ...
                    @error('gaji_pokok')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-muted">Gaji pokok sesuai dengan jabatan</small>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('tunjangan') is-invalid @enderror" 
                           ...
                           value="{{ old('tunjangan', 0) }}" min="0" step="1000">
                    @error('tunjangan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('potongan') is-invalid @enderror" 
                           ...
                           value="{{ old('potongan', 0) }}" min="0" step="1000">
                    @error('potongan')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-muted">Misal: BPJS, pajak, dll</small>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" class="form-control @error('total_gaji') is-invalid @enderror" 
                           ...
                           value="{{ old('total_gaji') }}" required min="0" step="1000">
                    @error('total_gaji')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                    <small class="text-muted">Gaji Pokok + Tunjangan - Potongan</small>
                </div>
